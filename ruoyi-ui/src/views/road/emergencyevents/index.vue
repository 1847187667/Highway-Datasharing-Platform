<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="事件发生路段" prop="eventLocation" label-width="100px">
          <el-select v-model="queryParams.eventLocation" placeholder="请选择事件发生路段">
            <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['road:emergencyevents:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['road:emergencyevents:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['road:emergencyevents:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['road:emergencyevents:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="emergencyeventsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="事件时间" align="center" prop="eventTime" width="200">
        <template slot-scope="scope">
      ...
      <el-table-column label="现场图片" align="center" prop="image" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.image" :width="60" :height="60"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="事件发生时间" prop="eventTime" label-width="100px">
          <el-date-picker clearable
        ...
        <el-form-item label="紧急事件类型" prop="eventType" label-width="100px">
        ...
        <el-form-item label="事件发生路段" prop="eventLocation" label-width="100px">
          <el-select v-model="form.eventLocation" placeholder="请选择事件发生路段">
            <el-option
              v-for="dict in dict.type.location"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listEmergencyevents, getEmergencyevents, delEmergencyevents, addEmergencyevents, updateEmergencyevents } from "@/api/road/emergencyevents";

export default {
  name: "Emergencyevents",
  dicts: ['dept_id', 'event_type','location'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listEmergencyevents(this.queryParams).then(response => {
        this.emergencyeventsList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加紧急事件数据管理";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getEmergencyevents(id).then(response => {
        this.form = response.data;
        this.deptList = response.data.deptList;
        this.open = true;
        this.title = "修改紧急事件数据管理";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateEmergencyevents(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addEmergencyevents(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除紧急事件数据管理编号为"' + ids + '"的数据项？').then(function() {
        return delEmergencyevents(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('road/emergencyevents/export', {
        ...this.queryParams
      }, `emergencyevents_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
